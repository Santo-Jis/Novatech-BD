    ...
    <div className="min-h-screen flex flex-col" style={{ background: 'linear-gradient(135deg, #1e3a8a 0%, #1e40af 50%, #1d4ed8 100%)' }}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '24px' }}>
        ...
        <button
          onClick={onLogin}
          disabled={loggingIn}
          style={{ width: '100%', maxWidth: 360, padding: '16px', borderRadius: 16, background: loggingIn ? 'rgba(255,255,255,0.7)' : 'white', border: 'none', color: '#1e3a8a', fontSize: 16, fontWeight: 700, cursor: loggingIn ? 'not-allowed' : 'pointer', boxShadow: '0 8px 24px rgba(0,0,0,0.2)', fontFamily: "'Hind Siliguri', sans-serif", display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, transition: 'transform 0.15s, opacity 0.2s', opacity: loggingIn ? 0.8 : 1 }}
        ...
        >
          {loggingIn ? (
            <>
              <span style={{ width: 18, height: 18, border: '2px solid rgba(30,58,138,0.2)', borderTop: '2px solid #1e3a8a', borderRadius: '50%', display: 'inline-block', animation: 'spin 0.8s linear infinite', flexShrink: 0 }} />
              লগইন হচ্ছে...
            </>
          ) : (
            <>
