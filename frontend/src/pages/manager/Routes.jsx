...
// Manager: রুট ম্যানেজমেন্ট
//
// নতুন যা যোগ হয়েছে:
//   - "Sequence সেট করুন" বাটন প্রতিটা route card-এ
//   - CustomerSequenceModal: drag-and-drop দিয়ে সাজানো
//   - "Live Status" বাটন: আজকে কে কোন route-এ — visit progress
//
// API endpoints (backend-এ নতুন):
//   GET  /api/routes/:id/customers  → route-র customer + visit_order
//   GET  /api/routes/live-status    → আজকের SR-route mapping
//   PUT  /api/customers/visit-order → sequence save (ছিলই)
...
import {
  ...
  FiChevronUp, FiClock, FiAlertCircle, FiActivity,
  FiMenu, FiSave, FiRefreshCw, FiHash, FiNavigation
} from 'react-icons/fi'

const fmt = n => Number(n || 0).toLocaleString('bn-BD')

// ── Customer Sequence Modal ───────────────────────────────────
      ...
      <div className="bg-white w-full max-w-lg rounded-t-3xl max-h-[90vh] flex flex-col">
        ...
        <div className="flex items-center justify-between px-5 pt-5 pb-3 border-b border-gray-100 flex-shrink-0">
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => { setCustomers(prev => prev.map((c, i) => ({ ...c, visit_order: i + 1 }))); setDirty(true) }}
        ...
        <div className="mx-4 mt-3 bg-blue-50 border border-blue-100 rounded-xl px-3 py-2 text-xs text-blue-700 flex items-center gap-2 flex-shrink-0">
          ...
          <span>কার্ড ধরে <strong>টানুন</strong> বা নম্বর বক্সে ক্রম লিখুন। SR-এর ফোনে এই ক্রমে দেখাবে।</span>
        </div>
          ...
          {loading ? (
            ...
            >
              ...
              {c.latitude && c.longitude && (
                <FiNavigation size={12} className="text-blue-400 flex-shrink-0" />
              )}
...
function LiveStatusPanel({ onClose }) {
  ...
  const grouped = data.reduce((acc, row) => {
    if (!acc[row.route_id]) acc[row.route_id] = { route_name: row.route_name, workers: [] }
      ...
      <div className="bg-white w-full max-w-lg rounded-t-3xl max-h-[85vh] flex flex-col">
        ...
        <div className="flex items-center justify-between px-5 pt-5 pb-3 border-b border-gray-100 flex-shrink-0">
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-bold text-gray-800 text-lg">আজকের Route Status</h2>
          </div>
          <div className="flex items-center gap-2">
            <button onClick={load} className="p-2 rounded-xl bg-gray-100 text-gray-500">
              <FiRefreshCw size={14} />
            </button>
            <button onClick={onClose} className="p-2 rounded-xl bg-gray-100 text-gray-500">
              <FiX />
              ...
              <div className="space-y-2">
                {group.workers.map(w => {
                  const pct = w.total_customers > 0
                    ? Math.round((w.visits_today / w.total_customers) * 100) : 0
                  ...
                  return (
                    <div key={w.worker_id}
                      ...
                      {w.last_seen_at && (
                        <p className="text-[10px] text-gray-400 flex-shrink-0">{timeAgo(w.last_seen_at)}</p>
                      )}
                    </div>
                  )
                })}
              ...
              <>
                ...
                {workers.map(w => (
                  <div key={w.id} className="flex items-center gap-3 p-3 rounded-xl border border-gray-100 hover:bg-gray-50">
                    ...
                    <button onClick={() => assign(w.id, w.name_bn)} disabled={assigning === w.id}
...
// ── Route Card ────────────────────────────────────────────────
function RouteCard({ route, onEdit, onDelete, onAssign, onSequence }) {
        ...
        <div className="border-t border-gray-50 px-4 py-3 flex gap-2 flex-wrap">
          ...
          <button
            ...
            className="flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-xl
                       bg-indigo-50 text-indigo-600 text-xs font-semibold border border-indigo-100"
          >
            <FiHash size={13} />
            Sequence সেট করুন
          </button>
          <button onClick={() => onAssign(route)}
            className="flex items-center justify-center gap-1.5 px-3 py-2.5 rounded-xl bg-primary/10 text-primary text-xs font-semibold">
            <FiUsers size={13} />
            SR
          </button>
...
export default function ManagerRoutes() {
  ...
  const [seqModal,    setSeqModal]    = useState(null)   // Sequence modal
  ...
  return (
    <div className="space-y-4 pb-6">
      ...
      {/* Modals */}
      {createModal && (
        <RouteModal onClose={() => setCreateModal(false)} onSuccess={load} />
      )}
      {editModal && (
        <RouteModal route={editModal} onClose={() => setEditModal(null)} onSuccess={load} />
      )}
      {assignModal && (
        <AssignWorkerModal route={assignModal} onClose={() => setAssignModal(null)} onSuccess={load} />
      )}
      {deleteModal && (
        <DeleteConfirm route={deleteModal} onClose={() => setDeleteModal(null)} onSuccess={load} />
      )}
      {seqModal && (
