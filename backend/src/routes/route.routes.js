...
const updateRoute = async (req, res) => {
    try {
        ...
        // ✅ FIX #3: Manager শুধু নিজের route update করতে পারবে
        // deleteRoute-এ এই check ছিল — updateRoute-এও একই pattern অনুসরণ করো।
        const isManager  = req.user.role === 'manager';
        const params     = [name, description, is_active, status, req.params.id];
        let managerClause = '';

        if (isManager) {
            managerClause = ' AND manager_id = $6';
        ...
        const result = await query(
            `UPDATE routes
             SET name        = COALESCE($1, name),
                 ...
                 updated_at  = NOW()
             WHERE id = $5${managerClause}
             RETURNING *`,
            params
        );

        if (result.rows.length === 0) {
            // manager_id mismatch হলেও 0 rows — 403 দাও
            if (isManager) {
                return res.status(403).json({ success: false, message: 'এই রুট আপনার নয়।' });
            }
